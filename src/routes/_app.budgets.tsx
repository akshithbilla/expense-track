...
function BudgetsPage() {
  ...
  const spent = useMemo(() => {
    return expenses
      .filter((e) => new Date(e.date) >= monthStart && e.status !== "Cancelled")
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2 overflow-hidden border-none shadow-elegant">
          <div className="relative bg-gradient-brand p-8 text-white">
            ...
            <p className="mt-1 font-display text-5xl tracking-tight">{formatMoney(budget, settings.currency)}</p>
            <div className="mt-6 space-y-2">
              ...
              <p className="text-sm opacity-90">{formatMoney(remaining, settings.currency)} remaining</p>
            </div>
          </div>
          <CardContent className="grid gap-4 p-6 sm:grid-cols-3">
            <MetricLine label="Daily budget" value={formatMoney(dailyBudget, settings.currency)} />
            <MetricLine label="Ideal spent" value={formatMoney(idealSpent, settings.currency)} sub={`by day ${daysPassed}`} />
            <MetricLine label="Status" value={onTrack ? "On track" : "Over pace"} accent={onTrack ? "success" : "warning"} />
          </CardContent>
        </Card>

        <Card className="shadow-soft">
          <CardHeader><CardTitle>Set your budget</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <label className="text-xs text-muted-foreground">Monthly amount</label>
              <Input type="number" value={value} onChange={(e) => setValue(e.target.value)} />
            </div>
            <Button className="w-full bg-gradient-brand text-white hover:opacity-90" onClick={save}>Save budget</Button>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <ThresholdCard label="80% warning" reached={pct >= 80} value={formatMoney(budget * 0.8, settings.currency)} />
        <ThresholdCard label="90% critical" reached={pct >= 90} value={formatMoney(budget * 0.9, settings.currency)} />
        <ThresholdCard label="100% exceeded" reached={pct >= 100} value={formatMoney(budget, settings.currency)} />
      </div>
    </div>
  );
}

function MetricLine({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: "success" | "warning" }) {
...
function ThresholdCard({ label, reached, value }: { label: string; reached: boolean; value: string }) {
  return (
    <Card className={`shadow-soft ${reached ? "border-warning/50 bg-warning/5" : ""}`}>
      <CardContent className="flex items-center gap-3 p-4">
        <div className={`grid h-10 w-10 place-items-center rounded-xl ${reached ? "bg-warning/20 text-warning" : "bg-accent text-muted-foreground"}`}>
