...
export const signIn = createServerFn({ method: "POST" }).validator(credentials.extend({ rememberMe: z.boolean().default(true) })).handler(async ({ data }) => (await import("./auth-service.server")).signIn(data));
